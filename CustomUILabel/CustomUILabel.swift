...
/**
 * The delegate of CustomUILabel.
 */
public protocol CustomUILabelDelegate: NSObjectProtocol {
 ...
 * CustomUILabel
 */
open class CustomUILabel: UILabel {
    ...
    /// The delegate of CustomUILabel
    weak open var delegate: CustomUILabelDelegate?
    ...
    fileprivate func textReplaceWordWithLink(_ lineIndex: LineIndexTuple, text: NSAttributedString, linkName: NSAttributedString) -> NSAttributedString {
        let lineText = text.text(for: lineIndex.line) //get text in lineIndex
    ...
    fileprivate func getCollapsedText(for text: NSAttributedString?, link: NSAttributedString) -> NSAttributedString? {
        ...
        if collapsedNumberOfLines > 0 && collapsedNumberOfLines < lines.count {
            ...
            if self.textReplacementType == .word { //here
            ...
            if let lineIndex = lineIndex, let modifiedLastLineText = modifiedLastLineText {
                ...
                print("range: \(collapsedLinkTextRange)")
    ...
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if !collapsed {
            ...
            if shouldCollapse && CustomUILabel.isTouchInLabelRange(touch: touch, label: self, inRange: range) {
    ...
    @discardableResult fileprivate func setLinkHighlighted(_ touches: Set<UITouch>?, event: UIEvent?, highlighted: Bool) -> Bool {
        ...
        if collapsed && CustomUILabel.isTouchInLabelRange(touch: touch, label: self, inRange: range) {
